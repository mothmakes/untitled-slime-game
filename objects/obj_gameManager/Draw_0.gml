...
if treed == true {
	...
	for (var i = 0;i<array_length_1d(tree);i++) {
		//Get start and end rooms for the current edge, draw line between them
...
//If graph completed, draw it.
	...
	for (var i = 0;i<instance_number(obj_room);i++) {
		var _node = rooms[i];
		var _edges = adjacency_list[?rooms[i]];
	
		for (var j = 0;j<instance_number(obj_room);j++) {
			if ds_map_exists(_edges,rooms[j]) {
				var _destroom = rooms[j];
				draw_line(_node.x,_node.y,_destroom.x,_destroom.y)
			}
		}
	}
}